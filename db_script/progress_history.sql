...
$$
  declare
     v_uid int;
  begin
     select into v_uid user_id from progress_history
        where user_id = p_uid;
...
--insert into progress_history values (1, 180, 67, 18.0, 'normal', 18, 0);
...
$$
    ...
    select user_id, height, weight, bmi, status, age, prog_perc from progress_history
    where user_id = $1;
    
$$
language 'sql';

--select * from progress_history where user_id = 1;
--getproghist(2);